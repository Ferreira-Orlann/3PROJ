...
class WorkspacesService {
    ...
    public async update(workspace: Partial<Workspace>) {
        const response = await axios.put<Workspace>("http://localhost:3000/workspaces", JSON.stringify(workspace), {
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${authService.getSession().token}`,
            }
    ...
    public async getByUUID(uuid: UUID): Promise<Workspace> {
        const response = await axios.get<Workspace>(`http://localhost:3000/workspaces/${uuid}`, {
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${authService.getSession().token}`,
            }
