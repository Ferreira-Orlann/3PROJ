...
const WebSocketDebugger = () => {
    ...
    useEffect(() => {
        ...
        const onMessage = (data: any) => {
            addLog(`Message reçu: ${JSON.stringify(data).substring(0, 100)}...`);
        };
        ...
        WebSocketService.addSocketListener('message.created', (data) => addLog(`message.created: ${JSON.stringify(data).substring(0, 100)}...`));
        WebSocketService.addSocketListener('message_sent', (data) => addLog(`message_sent: ${JSON.stringify(data).substring(0, 100)}...`));
        ...
        return () => {
            ...
            WebSocketService.removeSocketListener('connect_error', onConnectError);
            ...
            <View style={styles.buttonContainer}>
                <TouchableOpacity 
                    ...
                    onPress={() => {
                        if (token) {
                            WebSocketService.connect(token)
                                .then(() => setLogs(prev => [...prev, 'Reconnexion réussie']))
                                .catch(err => setLogs(prev => [...prev, `Erreur: ${err.message}`]));
                        }
                    }}
                >
