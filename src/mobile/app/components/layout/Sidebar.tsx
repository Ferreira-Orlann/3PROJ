...
export default function Sidebar() {
  ...
  const pathWorkspaceId = pathname.match(/\/screens\/workspaces\/([^/]+)/)?.[1] || null;
  ...
  useEffect(() => {
    ...
    if (filteredWorkspaces && Array.isArray(filteredWorkspaces) && filteredWorkspaces.length > 0) {
                ...
                <View key={workspaceId}>
                  ...
                  >
                    <View style={styles.workspaceAvatar}>
                      <Text style={styles.avatarText}>{workspace.name[0]}</Text>
                    </View>
                    {isExpanded && (
