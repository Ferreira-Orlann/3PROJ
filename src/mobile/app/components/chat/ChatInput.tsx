...
interface ChatInputProps {
  channelName?: string;
  placeholder?: string;
  onSendMessage: (message: string, attachments: ApiAttachment[]) => Promise<any>;
...
const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.error('ChatInput - Erreur lors de l\'envoi du message:', error);
      if (Platform.OS === 'android') {
        ToastAndroid.show('Erreur lors de l\'envoi du message. Veuillez réessayer.', ToastAndroid.SHORT);
      }
    } finally {
      setIsSending(false);
    }
  ...
  const handleAttachmentSelect = async (type: string) => {
    if (onPickFile) {
      ...
      const mockAttachments = {
        ...
        'document': { type: 'document', name: 'document.pdf', size: '2.3 MB' },
      };
      
      setAttachments([...attachments, mockAttachments[type as keyof typeof mockAttachments]]);
    }
    setShowAttachmentOptions(false);
  };
  ...
  return (
    <>
      ...
      {replyMessage && (
        <View style={styles.replyContainer}>
          <View style={styles.replyContent}>
            <Text style={styles.replyLabel}>Replying to <Text style={styles.replySender}>{typeof replyMessage.source === 'object' && replyMessage.source?.username ? replyMessage.source.username : 'Unknown'}</Text></Text>
            <Text style={styles.replyMessage} numberOfLines={1}>{replyMessage.message}</Text>
          </View>
      ...
      <View style={styles.inputContainer}>
        ...
        <View style={[styles.connectionIndicator, isWebSocketConnected ? styles.connected : styles.disconnected]} />
        
        {/* Attachment button */}
        <TouchableOpacity onPress={toggleAttachmentOptions} style={styles.iconButton}>
        ...
        {/* Emoji button */}
        <TouchableOpacity onPress={toggleEmojiPicker} style={styles.iconButton} disabled={isSending}>
        ...
        {isSending ? (
          ...
          >
            <Ionicons name="send" size={20} color={message.trim() ? "#fff" : "#8e9297"} />
          </TouchableOpacity>
        )}
      </View>
