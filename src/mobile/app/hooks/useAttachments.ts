...
import attachmentService, {
    Attachment,
} from "../services/api/endpoints/attachments";
...
export const useAttachments = (
    ...
    const uploadFile = useCallback(
        async (fileUri: string, fileName: string, fileType: string) => {
            ...
            try {
                ...
                // Upload du fichier
                const fileUuid = await attachmentService.uploadFile(
                    formData as any,
                );
        ...
        async (
            ...
            try {
                ...
                if (workspaceUuid && channelUuid) {
                    ...
                    throw new Error(
                        "Soit workspaceUuid soit userUuid doit être fourni avec channelUuid",
                    );
                }
                ...
                console.error(
                    "Erreur lors de l'upload et l'attachement du fichier:",
                    err,
                );
                setError(
                    "Impossible d'attacher le fichier au message. Veuillez réessayer.",
                );
                return null;
            } finally {
                setUploading(false);
    ...
    const downloadFile = useCallback(
        async (fileUuid: UUID, filename: string) => {
            setLoading(true);
            setError(null);
            ...
            try {
                if (!FileSystem || !Sharing) {
                    Alert.alert(
                        "Erreur",
                        "Le téléchargement de fichiers n'est pas disponible",
                    );
                    return null;
                }
                ...
                if (downloadResult.status !== 200) {
                    throw new Error(
                        `Échec du téléchargement: ${downloadResult.status}`,
                    );
                }
    ...
    const removeAttachment = useCallback(
        async (attachmentUuid: UUID) => {
            ...
            try {
                ...
                setAttachments((prev) =>
                    prev.filter((att) => att.uuid !== attachmentUuid),
                );
                ...
                setError(
                    "Impossible de supprimer la pièce jointe. Veuillez réessayer.",
                );
                return false;
            }
        },
