...
import channelService, { CreateChannelData } from '../services/api/endpoints/channels';
...
const useWorkspaceScreen = (workspaceId: UUID) => {
      ...
      try {
        ...
        try {
          ...
          members = apiMembers.map(apiMember => {
            ...
            const mappedStatus = userStatus === 'en ligne' ? 'online' :
                                userStatus === 'absent' ? 'away' :
                                userStatus === 'hors ligne' ? 'offline' : 'online';
  ...
  const handleChannelSelect = (channelId: UUID) => {
    router.push({ pathname: '/screens/workspaces/[id]/channels/[channelId]', params: { id: workspaceId, channelId } });
  };
  ...
  const handleRemoveMember = async (memberId: UUID) => {
    try {
      ...
      Alert.alert("Succès", "Le membre a été retiré de l'espace de travail.");
    } catch (err) {
  ...
  const updateNewChannelForm = (field: keyof NewChannelForm, value: string | boolean) => {
