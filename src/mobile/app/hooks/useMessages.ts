...
import messageService, { Message, CreateMessageData } from '../services/api/endpoints/messages';
 ...
 */
export const useMessages = (workspaceUuid: UUID | null, channelUuid: UUID, userUuid: UUID, currentUserUuid: UUID) => {
  ...
  let currentUser = { uuid: currentUserUuid } as { uuid: UUID; username: string } | null;
  if (useAuth) {
    try {
  ...
  useEffect(() => {
    ...
    if (!initialConnected) {
      console.log('useMessages - WebSocket non connecté, tentative de connexion...');
      ...
      const getTokenAndConnect = async () => {
        try {
          ...
          if (token) {
            console.log('useMessages - Token récupéré, tentative de connexion WebSocket');
            websocketService.connect(token).catch(error => {
              console.error('useMessages - Impossible de se connecter au WebSocket:', error);
            });
          } else {
            console.error('useMessages - Impossible de se connecter au WebSocket: token manquant');
            // Essayer de récupérer le token depuis useAuth si disponible
            if (currentUser?.uuid) {
              console.log('useMessages - Tentative de récupération du token depuis currentUser');
              // Utiliser l'UUID comme token temporaire si nécessaire
              const tempToken = currentUser.uuid.toString();
              websocketService.connect(tempToken).catch(error => {
                console.error('useMessages - Impossible de se connecter au WebSocket avec UUID:', error);
              });
            }
          }
        } catch (error) {
          console.error('useMessages - Erreur lors de la récupération du token:', error);
        }
      };
    ...
    const checkInterval = setInterval(() => {
      ...
      if (connected !== isWebSocketConnected) {
        console.log(`useMessages - État WebSocket changé: ${connected ? 'connecté' : 'déconnecté'}`);
  ...
  const fetchMessages = useCallback(async () => {
    if (!channelUuid) {
      console.log('useMessages - fetchMessages - Impossible de charger les messages: channelUuid manquant');
      return;
    }
    ...
    try {
      ...
      if (workspaceUuid) {
        ...
        try {
          fetchedMessages = await messageService.getMessages(workspaceUuid, channelUuid);
        } catch (apiError: any) {
          console.error('useMessages - fetchMessages - Erreur API workspace:', apiError);
          if (apiError?.message?.includes('ECONNREFUSED') || apiError?.code === 'ERR_CONNECTION_REFUSED') {
        ...
        // Messages privés
        try {
          fetchedMessages = await messageService.getDirectMessages(userUuid, channelUuid);
        } catch (apiError: any) {
          console.error('useMessages - fetchMessages - Erreur API messages privés:', apiError);
          if (apiError?.message?.includes('ECONNREFUSED') || apiError?.code === 'ERR_CONNECTION_REFUSED') {
            throw new Error('Impossible de se connecter au serveur. Vérifiez que le serveur est en cours d\'exécution.');
          }
          throw apiError;
      ...
      console.log(`useMessages - fetchMessages - ${fetchedMessages.length} messages chargés`);
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    if (!channelUuid) {
      console.error('useMessages - sendMessage - Erreur: channelUuid manquant');
      return null;
    }
    
    if (!content.trim()) {
      console.error('useMessages - sendMessage - Erreur: contenu vide');
      return null;
    }
    
    if (!currentUser?.uuid) {
      console.error('useMessages - sendMessage - Erreur: UUID utilisateur manquant');
      return null;
    }
    ...
    try {
      ...
      if (isWebSocketConnected) {
        try {
          if (workspaceUuid) {
            websocketService.sendMessageToChannel(channelUuid, content);
          } else if (userUuid) {
            websocketService.sendDirectMessage(userUuid, content);
          }
          // Continuer avec l'API REST pour s'assurer que le message est bien envoyé
        } catch (wsError) {
          console.error('useMessages - sendMessage - Erreur WebSocket, utilisation de l\'API REST:', wsError);
        }
      }
      ...
      try {
        if (workspaceUuid) {
          ...
          console.log(`useMessages - sendMessage - Envoi du message privé à l'utilisateur ${userUuid}, canal ${channelUuid}`);
        ...
        console.error('useMessages - sendMessage - Erreur API:', apiError);
        ...
        if (isWebSocketConnected) {
          ...
          console.log('useMessages - sendMessage - Message envoyé via WebSocket uniquement, création d\'un message temporaire');
        } else {
      ...
      console.error('useMessages - sendMessage - Erreur lors de l\'envoi du message:', err);
   ...
   */
  const editMessage = useCallback(async (messageUuid: UUID, content: string) => {
    ...
    try {
      ...
      setMessages(prev => 
        prev.map(msg => msg.uuid === messageUuid ? updatedMessage : msg)
      );
   ...
   * Supprime un message
   */
  const deleteMessage = useCallback(async (messageUuid: UUID) => {
  ...
  useEffect(() => {
    if (!channelUuid) {
      console.log('useMessages - useEffect WebSocket - Impossible de s\'abonner: channelUuid manquant');
      return;
    }
    
    if (!isWebSocketConnected) {
      console.log('useMessages - useEffect WebSocket - WebSocket non connecté, en attente de connexion...');
      return;
    }

    console.log(`useMessages - useEffect WebSocket - Abonnement aux événements pour le canal ${channelUuid}`);
    ...
    const onMessageCreated = websocketService.on('message_sent', (data: Message) => {
      // Vérifier si le message appartient au canal actuel
      const isForCurrentChannel = 
        (data.destination_channel && data.destination_channel.uuid === channelUuid) ||
        (data.destination_user && data.destination_user.uuid === channelUuid);
      
      if (isForCurrentChannel) {
        console.log('useMessages - WebSocket - Nouveau message pour ce canal:', data);
        setMessages(prev => {
          // Éviter les doublons
    ...
    const onMessageUpdated = websocketService.on('message_updated', (data: Message) => {
      // Vérifier si le message appartient au canal actuel
      const isForCurrentChannel = 
        (data.destination_channel && data.destination_channel.uuid === channelUuid) ||
        (data.destination_user && data.destination_user.uuid === channelUuid);
      ...
      if (isForCurrentChannel) {
        setMessages(prev => 
          prev.map(msg => msg.uuid === data.uuid ? data : msg)
        );
      }
    });

    // Gérer les suppressions de messages - écouter à la fois 'message_removed' et 'message.removed'
    const onMessageRemoved = websocketService.on('message_removed', (data: { messageUuid: UUID }) => {
      setMessages(prev => prev.filter(msg => msg.uuid !== data.messageUuid));
    });
    
    // Écouter également l'événement générique 'message_sent' du socket.io
    const onSocketMessage = websocketService.on('message_sent', (data: any) => {
      if (data && data.event && data.payload) {
        console.log(`useMessages - Socket.io - Événement générique reçu: ${data.event}`, data.payload);
    ...
    return () => {
      console.log(`useMessages - useEffect WebSocket - Désabonnement des événements pour le canal ${channelUuid}`);
      onMessageCreated();
      onMessageUpdated();
      onMessageRemoved();
      onSocketMessage();
