...
const convertApiMessageToUiMessage = (
    ...
    currentUserUuid: UUID | null = null,
): Message => {
    const sender =
        typeof apiMessage.source === "string"
            ? "Utilisateur inconnu"
    ...
    return {
        ...
        reactions: Array.isArray(apiMessage.createdReaction)
            ? apiMessage.createdReaction.map((r: ApiReaction) => ({
                  emoji: r.emoji,
                  count: 1,
                  users: [r.user.uuid],
              }))
...
export const useDirectMessage = (userId: string) => {
    ...
    useEffect(() => {
        if (currentUser && currentUser.uuid) {
            ...
            console.log(
                "UUID de l'utilisateur courant (depuis currentUser):",
                currentUser.uuid,
            );
        }
    }, [currentUser]);
    ...
    useEffect(() => {
        if (token) {
            console.log(
                "useDirectMessage - Initialisation de la connexion WebSocket",
            );
            WebSocketService.connect(token)
                .then(() =>
                    console.log(
                        "useDirectMessage - WebSocket connecté avec succès",
                    ),
                )
                .catch((err) =>
                    console.error(
                        "useDirectMessage - Erreur de connexion WebSocket:",
                        err,
                    ),
                );
        }
    ...
    console.log(
        "useDirectMessage - UUID utilisateur courant (de useAuth):",
        currentUserUuid,
    );
    ...
    useEffect(() => {
        const fetchRecipientUser = async () => {
            if (!userId) {
                console.error(
                    "useDirectMessage - Aucun userId fourni pour récupérer l'utilisateur destinataire",
                );
            ...
            console.log(
                "useDirectMessage - Récupération du destinataire avec ID:",
                userId,
            );
            ...
            try {
                ...
                console.log(
                    "useDirectMessage - Données du destinataire récupérées:",
                    recipientData,
                );
                ...
                console.log(
                    "useDirectMessage - Utilisateur destinataire créé:",
                    newUser,
                );
                setUser(newUser);
                ...
                console.error(
                    "Erreur lors de la récupération du destinataire:",
                    error,
                );
                setError(
    ...
    useEffect(() => {
        if (user) {
            console.log(
                "useDirectMessage - Utilisateur destinataire mis à jour:",
                user,
            );
        }
    }, [user]);
    ...
    useEffect(() => {
        console.log("useDirectMessage - Utilisateur courant:", currentUser);
        console.log(
            "useDirectMessage - UUID utilisateur courant:",
            currentUserUuid,
        );
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            if (!userId) {
                console.error(
                    "useDirectMessage - Aucun userId fourni pour récupérer les messages",
                );
                return;
            }
            ...
            if (!currentUser || !currentUserUuid) {
                console.error(
                    "useDirectMessage - Utilisateur courant non disponible pour récupérer les messages",
                );
            ...
            console.log(
                "useDirectMessage - Récupération des messages entre",
            ...
            try {
                ...
                const recipientId = userId as UUID;
                console.log(
                    "useDirectMessage - Appel API getDirectMessages avec ID destinataire:",
                    recipientId,
                );

                // Récupérer les messages reçus par l'utilisateur destinataire
                const receivedMessages =
                    await messageService.getDirectMessages(recipientId);
                console.log(
                    "useDirectMessage - Messages reçus par le destinataire:",
                    receivedMessages,
                );
                ...
                if (currentUserUuid) {
                    try {
                        sentMessages = await messageService.getDirectMessages(
                            currentUserUuid as UUID,
                        );
                        console.log(
                            "useDirectMessage - Messages reçus par l'utilisateur courant:",
                            sentMessages,
                        );
                    } catch (error) {
                        console.error(
                            "Erreur lors de la récupération des messages de l'utilisateur courant:",
                            error,
                        );
                    }
                }
                ...
                const allMessages = [
                    ...
                    const sourceId =
                        typeof msg.source === "string"
                            ...
                            : msg.source.uuid;
                    const destId =
                        typeof msg.destination_user === "string"
                            ? msg.destination_user
                            : msg.destination_user?.uuid;

                    return (
                        (sourceId === currentUserUuid &&
                            destId === recipientId) ||
                ...
                const messagesData = allMessages.sort(
                    (a, b) =>
                        new Date(a.date).getTime() - new Date(b.date).getTime(),
                );

                console.log(
                    "useDirectMessage - Messages combinés et filtrés:",
                    messagesData,
                );
                ...
                console.log(
                    "useDirectMessage - Messages convertis pour l'UI:",
                    convertedMessages,
                );
        ...
        console.log(
            "Configuration des écouteurs WebSocket dans useDirectMessage",
        );
        ...
        const messageHandler = (data: any) => {
            console.log(
                "WebSocket - Événement 'message' reçu directement:",
                JSON.stringify(data),
            );
        ...
        const messageCreatedHandler = (data: any) => {
            console.log(
                "WebSocket - Événement 'message.created' reçu directement:",
                JSON.stringify(data),
            );
            handleNewMessage(data);
        };
        ...
        WebSocketService.addSocketListener(
            "message.created",
    ...
    const handleNewMessage = useCallback(
        (data: any) => {
            console.log(
                "WebSocket - Message reçu dans useDirectMessage:",
                JSON.stringify(data),
            );
            ...
            if (!message || !message.uuid) {
                console.log(
                    "WebSocket - Message invalide ou sans UUID, ignoré",
                );
                return;
            }

            console.log(
                "WebSocket - Message formaté:",
            ...
            console.log(
                "WebSocket - isForThisConversation:",
            ...
            if (isForThisConversation) {
                ...
                setMessages((prevMessages) => {
                    ...
                    const messageExists = prevMessages.some(
                        (msg) => msg.id === message.uuid,
                    );
                    console.log(
                        "WebSocket - Le message existe déjà?",
                        messageExists,
                    );
                    if (messageExists) {
                        console.log(
                            "WebSocket - Message déjà existant, pas d'ajout",
                        );
                        return prevMessages;
                    }

                    // Convertir le message API en message UI
                    const uiMessage = convertApiMessageToUiMessage(
                        message,
                        currentUserUuid,
                    );
                    console.log(
                        "WebSocket - Nouveau message ajouté:",
                ...
                setTimeout(() => {
                    console.log(
                        "WebSocket - Forçage du rafraîchissement de l'interface",
                    );
                    setRefreshTrigger((prev) => !prev);

                    // Afficher le nombre de messages après l'ajout
                    console.log(
                        "WebSocket - Nombre de messages après ajout:",
                        messages.length + 1,
                    );
                }, 50);
            } else {
                console.log(
                    "WebSocket - Message ignoré (pas pour cette conversation)",
                );
            }
        },
        [userId, currentUserUuid],
    ...
    const handleSendMessage = async (
        ...
        if (!content.trim() && attachments.length === 0) {
            console.log(
                "useDirectMessage - Tentative d'envoi d'un message vide, annulé",
            );
            return;
        }

        console.log(
            "useDirectMessage - Tentative d'envoi de message à l'utilisateur:",
            userId,
        );
        ...
        if (!currentUser || !currentUserUuid) {
            console.error(
                "useDirectMessage - Impossible d'envoyer le message: utilisateur courant non identifié",
            );
            alert(
                "Impossible d'envoyer le message: veuillez vous reconnecter et réessayer.",
            );
            return;
        }
        ...
        if (!userId) {
            console.error(
                "useDirectMessage - Impossible d'envoyer le message: destinataire non identifié",
            );
            alert(
                "Impossible d'envoyer le message: destinataire non identifié.",
            );
            return;
        }
        ...
        console.log(
            "useDirectMessage - Création d'un message optimiste avec ID temporaire:",
            tempId,
        );
        ...
        try {
            ...
            if (!sourceUuid || typeof sourceUuid !== "string") {
                console.error(
                    "useDirectMessage - Aucun ID utilisateur valide disponible:",
                    { currentUserUuid },
                );
                throw new Error(
            ...
            if (!recipientId || typeof recipientId !== "string") {
                console.error(
                    "useDirectMessage - UUID du destinataire invalide:",
                    recipientId,
                );
            ...
            console.log(
                "useDirectMessage - Préparation de l'envoi du message:",
            );
            ...
            if (isValidUUID(sourceUuid)) {
                ...
                console.warn(
                    "useDirectMessage - sourceUuid n'est pas un UUID valide, utilisation telle quelle",
                );
            ...
            if (isValidUUID(recipientId)) {
                ...
                console.warn(
                    "useDirectMessage - recipientId n'est pas un UUID valide, utilisation telle quelle",
                );
            ...
            console.log(
                "useDirectMessage - Données du message à envoyer:",
            ...
            if (!formattedSourceUuid || !formattedDestinationUuid) {
                throw new Error(
                    "Les UUIDs source ou destination sont invalides",
                );
            }

            // Utiliser la nouvelle API avec le format users/recepteurId/messages
            console.log(
                `useDirectMessage - Appel API POST /users/${recipientId}/messages`,
            );
            const response = await messageService.sendDirectMessage(
                formattedDestinationUuid,
                messageData,
            );

            console.log(
                "useDirectMessage - Message envoyé avec succès, réponse:",
                response,
            );
            ...
            console.error(
                "useDirectMessage - Erreur lors de l'envoi du message:",
                error,
            );
            ...
            if (error.response) {
                ...
                console.error(
                    "useDirectMessage - Détails de l'erreur serveur:",
                    {
                ...
                console.error(
                    "useDirectMessage - Erreur de requête (pas de réponse):",
                    error.request,
                );
            } else {
                // Autre type d'erreur
                console.error(
                    "useDirectMessage - Message d'erreur:",
                    error.message,
                );
            }
            ...
            setError(
                "Erreur lors de l'envoi du message. Vérifiez les logs pour plus de détails.",
            );
    ...
    const handleAddReaction = async (messageId: string, emoji: string) => {
        if (!currentUserUuid) {
            console.error(
                "Impossible d'ajouter une réaction: utilisateur non identifié",
            );
            return;
        }
    ...
    const handleRemoveReaction = async (messageId: string, emoji: string) => {
        if (!currentUserUuid) {
            console.error(
                "Impossible de supprimer une réaction: utilisateur non identifié",
            );
            return;
        }
    ...
    useEffect(() => {
        console.log(
            "useDirectMessage - État refreshTrigger changé:",
            refreshTrigger,
        );
        console.log(
            "useDirectMessage - Nombre de messages actuel:",
            messages.length,
        );
