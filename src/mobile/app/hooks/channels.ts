...
import channelService, { Channel as ApiChannel } from '../services/api/endpoints/channels';
...
export const useChannelScreen = (workspaceId: string, channelId: string) => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      setIsLoading(true);
      try {
        const messageData = await messageService.getMessages(workspaceId as UUID, channelId as UUID);
  ...
  const handleSendMessage = async (content: string, attachments: File[] = []) => {
    ...
    try {
      ...
      const newMessage: Message = {
        ...
        avatar: null,
        timestamp: new Date(sentMessage.createdAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
        reactions: [],
  ...
  const handleAddReaction = async (messageId: string, emoji: string) => {
    try {
      ...
      await messageService.addReaction(workspaceId as UUID, channelId as UUID, messageId as UUID, emoji);
      
      // Mettre à jour l'état local
      const updatedMessages = messages.map(msg => {
        if (msg.id === messageId) {
          const existingReactionIndex = msg.reactions.findIndex((r: MessageReaction) => r.emoji === emoji);
