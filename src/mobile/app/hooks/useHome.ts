...
export const useHomeScreen = () => {
    ...
    const fetchWorkspaces = async () => {
        try {
            ...
            const uiWorkspaces: Workspace[] = apiWorkspaces.map(
                (workspace) => ({
    ...
    const updateNewWorkspaceForm = (
        field: keyof CreateWorkspaceForm,
    ...
    const handleCreateWorkspace = async () => {
        ...
        if (!auth || !auth.user || !auth.user.uuid) {
            Alert.alert(
                "Erreur",
                "Vous devez être connecté pour créer un espace de travail.",
            );
            return;
        }
        ...
        try {
            ...
            if (workspaceId) {
                ...
                console.error(
                    "Could not determine workspace ID from response:",
                    apiWorkspace,
                );
                Alert.alert(
                    "Information",
                    "L'espace de travail a été créé, mais la navigation automatique a échoué. Veuillez rafraîchir la liste des espaces de travail.",
                );
            }
        } catch (error) {
