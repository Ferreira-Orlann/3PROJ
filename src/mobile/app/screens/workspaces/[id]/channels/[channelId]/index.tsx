...
export default function ChannelScreen() {
  ...
  useEffect(() => {
    if (!token) {
      console.log('ChannelScreen - Aucun token d\'authentification disponible');
      return;
    }
  ...
  useEffect(() => {
    const loadWorkspaceAndChannel = async () => {
      ...
      try {
        ...
        const workspaceData = await workspaceService.getWorkspaceByUuid(workspaceId as UUID);
        setWorkspace(workspaceData);
        
        // Charger les informations du channel
        const channelData = await channelService.getChannelById(channelId as UUID);
        setChannel(channelData);
        
      } catch (error) {
        console.error('ChannelScreen - Erreur lors du chargement des données:', error);
        console.error('Stack trace:', error instanceof Error ? error.stack : 'Pas de stack trace');
      } finally {
        console.log('ChannelScreen - Fin du chargement, isLoading:', false);
        setIsLoading(false);
      }
    };
    ...
    if (workspaceId && channelId) {
      ...
      console.log('ChannelScreen - workspaceId ou channelId manquant, pas de chargement');
    }
  ...
  useEffect(() => {
    if (workspaceId && channelId) {
      fetchMessages()
        .then(() => console.log('ChannelScreen - Messages chargés avec succès'))
        .catch(err => console.error('ChannelScreen - Erreur lors du chargement des messages:', err));
    } else {
      console.log('ChannelScreen - workspaceId ou channelId manquant, pas de chargement des messages');
    }
  ...
  const handleSendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      console.error('ChannelScreen - Erreur lors de l\'envoi du message:', error);
      console.error('Stack trace:', error instanceof Error ? error.stack : 'Pas de stack trace');
    }
  }, [sendMessage]);
  ...
  const handleAddReaction = useCallback(async (messageId: string, emoji: string) => {
    ...
    console.log('ChannelScreen - handleAddReaction - Début avec params:', { messageId, emoji });
    console.log('ChannelScreen - handleAddReaction - Implémentation à compléter avec l\'API appropriée');
          ...
          <View style={styles.mainContent}>
            <View style={styles.errorContainer}>
              <Text style={styles.errorText}>
                {isLoading ? 'Chargement...' : 'Canal non trouvé'}
              </Text>
              {!isLoading && (
                <TouchableOpacity 
                  style={styles.backButton}
                  onPress={() => {
                    console.log('ChannelScreen - Bouton Retour pressé');
                    router.back();
                  }}
                >
  ...
  return (
    <>
      ...
      >
        ...
        <View style={styles.header}>
          <View style={styles.channelInfo}>
            <View style={styles.channelNameContainer}>
              <Ionicons 
                name={channel.is_public ? "chatbubble" : "lock-closed"} 
                size={18} 
                color="#fff" 
          ...
          <View style={styles.headerActions}>
            <TouchableOpacity style={styles.headerButton}>
              <Ionicons name="people" size={22} color="#8e9297" />
            </TouchableOpacity>
            <TouchableOpacity style={styles.headerButton}>
              <Ionicons name="search" size={22} color="#8e9297" />
            </TouchableOpacity>
            <TouchableOpacity style={styles.headerButton}>
              <Ionicons name="notifications" size={22} color="#8e9297" />
            </TouchableOpacity>
          </View>
        </View>
        ...
        {(() => {
          console.log('ChannelScreen - ChatContainer rendu avec props:', { 
            workspaceUuid: workspaceId, 
            userUuid: workspaceId, /* Utiliser workspaceId comme userUuid temporairement */
        ...
        <ChatContainer
          ...
          userUuid={user?.uuid as UUID} /* Utiliser workspaceId comme userUuid temporairement */
