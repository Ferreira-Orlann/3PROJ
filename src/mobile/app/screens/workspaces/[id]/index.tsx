...
export default function WorkspaceScreen() {
  ...
  useEffect(() => {
    const fetchWorkspaceData = async () => {
      ...
      setIsLoading(true);
      console.log('Tentative de récupération du workspace avec ID:', workspaceId);
      
      try {
        const uuid = String(workspaceId);
        const workspaceData = await workspaceService.getWorkspaceByUuid(uuid as UUID);
        console.log('Données du workspace récupérées:', workspaceData);
        
        const allChannels = await channelService.getChannels();
        const channels = allChannels.filter(channel => channel.workspace === uuid); 

        console.log('Canaux récupérés:', channels);
        
        let members: Member[] = [];
        try {
          const apiMembers = await memberService.getWorkspaceMembers(uuid as UUID);
        ...
        setError('Impossible de charger les données de l\'espace de travail.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const filteredChannels = workspace?.channels?.filter((channel: Channel) => 
    channel.name && searchQuery ? channel.name.toLowerCase().includes(searchQuery.toLowerCase()) : true
  ) || [];

  const filteredMembers = workspace?.members?.filter((member: Member) => {
    const displayName = member.name || member.user.username || '';
    return searchQuery ? displayName.toLowerCase().includes(searchQuery.toLowerCase()) : true;
  }) || [];
  
  const handleOpenDirectMessage = (member: Member) => {    
    const dmId = `${member.user.uuid}`; 
    router.push({ pathname: '/screens/private_messages/[userId]', params: { userId: dmId } });
  };
  ...
  const handleCreateChannel = async () => {
    ...
    try {
      ...
      if (!channelUuid) {
        console.error('Erreur: UUID du canal créé non disponible', createdChannel);
  ...
  const handleRemoveChannel = async (channelId: UUID) => {
    try {
      ...
      setWorkspace({
        ...workspace,
        channels: workspace.channels.filter(c => c.uuid !== channelId),
  ...
  const handleUpdateWorkspaceInfo = async () => {
    try {
      ...
      const updatedWorkspace = await workspaceService.updateWorkspace(workspaceId as UUID, workspaceData);
      ...
      window.alert("Les informations de l'espace de travail ont été mises à jour avec succès.");
    } catch (err) {
      console.error('Erreur lors de la mise à jour du workspace:', err);
      window.alert("Impossible de mettre à jour les informations de l'espace de travail.");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
          ...
          <View style={styles.mainContent}>
            <View style={styles.errorContainer}>
              <Text style={styles.errorText}>Espace de travail non trouvé</Text>
              <TouchableOpacity 
    ...
    <TouchableOpacity 
     ...
     >
      <View style={styles.channelIconContainer}>
        <View style={[styles.channelIconBadge, { backgroundColor: item.is_public ? '#43b581' : '#faa61a' }]}>
          <Ionicons 
            name={item.is_public ? "apps-outline" : "lock-closed-outline"} 
            size={24} 
            color="#fff" 
          />
        </View>
      ...
      <View style={styles.channelContent}>
        ...
        <View style={styles.channelMeta}>
          <View style={styles.channelMetaItem}>
            ...
            <Text style={styles.channelMetaText}>{item.memberCount}</Text>
          </View>
        ...
        >
          <Ionicons name="remove-circle-outline" size={22} color="#8e9297" />
        </TouchableOpacity>
      </View>
        </View>
      </View>
    ...
    >
      <View style={styles.memberAvatarContainer}>
        <View 
          style={[
            ...
            { 
              borderColor: 
                displayStatus === 'en ligne' ? '#43b581' : 
                displayStatus === 'absent' ? '#faa61a' : '#747f8d' 
            }
          ]}
        >
          <Text style={styles.memberAvatarText}>{displayName.charAt(0)}</Text>
        </View>
        <View 
          style={[
      ...
      <View style={styles.memberInfo}>
        ...
        <Text style={styles.memberName}>{displayName}</Text>
          <View style={[styles.memberRoleBadge, { backgroundColor: item.role === 'admin' ? '#ed4245' : '#0721f8' }]}>
            <Text style={styles.memberRoleText}>{item.role || 'member'}</Text>
          </View>
        </View>
              ...
              <View style={styles.workspaceAvatarContainer}>
                ...
                <View style={styles.badgeContainer}>
                  <View 
                    style={[
                      styles.statusBadge, 
                      { backgroundColor: workspace.is_public ? '#43b581' : '#faa61a' }
                    ]} 
                  >
              ...
              <View style={styles.workspaceInfoContainer}>
                ...
                <View style={styles.workspaceStats}>
                  <View style={styles.statItem}>
                    <Ionicons name="people-outline" size={16} color="#8e9297" />
                    <Text style={styles.statText}>{workspace.members.length} membres</Text>
                  </View>
                  <View style={styles.statItem}>
                    <Ionicons name="calendar-outline" size={16} color="#8e9297" />
                    <Text style={styles.statText}>Créé le {workspace.createdAt}</Text>
                  </View>
                </View>
              </View>
            </View>
              ...
              >
                <View style={styles.tabContent}>
                  <Ionicons 
                    ...
                    size={22} 
                    color={activeTab === tab.id ? Colors.primary : '#8e9297'} 
                  />
                  <Text 
                    style={[
                      styles.tabText,
            ...
            {activeTab === 'settings' && (
              <ScrollView style={styles.settingsContainer} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.settingsHeader}>
                  ...
                  <Text style={styles.settingsHeaderTitle}>Paramètres de l'espace de travail</Text>
                </View>
                ...
                <View style={styles.settingSection}>
                    ...
                    {isEditingInfo && (
                      <View style={styles.editActions}>
                        <TouchableOpacity 
                          ...
                          onPress={handleUpdateWorkspaceInfo}
                        >
                  ...
                  <View style={styles.settingCard}>
                    <View style={styles.settingItem}>
                      <Text style={styles.settingLabel}>Nom de l'espace</Text>
                      <View style={styles.inputContainer}>
                        <TextInput
                          style={[styles.settingInput, isEditingInfo && styles.activeInput]}
                          value={editedWorkspaceInfo.name}
                          onChangeText={(text) => setEditedWorkspaceInfo({...editedWorkspaceInfo, name: text})}
                          placeholder="Nom de l'espace de travail"
                          placeholderTextColor="#8e9297"
                          editable={isEditingInfo}
                        />
                        {isEditingInfo && <Ionicons name="pencil-outline" size={20} color={Colors.primary} style={styles.inputIcon} />}
                      </View>
                    </View>
                    ...
                    <View style={styles.settingItem}>
                      ...
                      <View style={styles.inputContainer}>
                        <TextInput
                          style={[styles.settingInput, styles.textArea, isEditingInfo && styles.activeInput]}
                          value={editedWorkspaceInfo.description}
                          onChangeText={(text) => setEditedWorkspaceInfo({...editedWorkspaceInfo, description: text})}
                          ...
                          editable={isEditingInfo}
                        />
                        {isEditingInfo && <Ionicons name="pencil-outline" size={20} color={Colors.primary} style={[styles.inputIcon, { alignSelf: 'flex-start', marginTop: 8 }]} />}
                      </View>
                    </View>
                      ...
                      <View style={styles.visibilityOptions}>
                        <TouchableOpacity 
                          style={[
                            ...
                            editedWorkspaceInfo.is_public && styles.selectedVisibility
                          ]}
                          onPress={() => isEditingInfo && setEditedWorkspaceInfo({...editedWorkspaceInfo, is_public: true})}
                          disabled={!isEditingInfo}
                        >
                          <Ionicons name="globe-outline" size={20} color={editedWorkspaceInfo.is_public ? '#fff' : '#8e9297'} />
                          <Text style={[
                            styles.visibilityText,
                            editedWorkspaceInfo.is_public && styles.selectedVisibilityText
                          ]}>Public</Text>
                        </TouchableOpacity>
                        ...
                        <TouchableOpacity 
                          ...
                          onPress={() => isEditingInfo && setEditedWorkspaceInfo({...editedWorkspaceInfo, is_public: false})}
                          disabled={!isEditingInfo}
                        >
                          <Ionicons name="lock-closed-outline" size={20} color={!editedWorkspaceInfo.is_public ? '#fff' : '#8e9297'} />
                          <Text style={[
                            styles.visibilityText,
                            !editedWorkspaceInfo.is_public && styles.selectedVisibilityText
                          ]}>Privé</Text>
                ...
                <View style={styles.settingSection}>
                  ...
                  <View style={styles.settingCard}>
                    <View style={styles.settingItem}>
                      <View>
                        <Text style={styles.settingLabel}>Qui peut créer des canaux</Text>
                        <Text style={styles.settingSubLabel}>Définir qui peut ajouter de nouveaux canaux</Text>
                      </View>
                      <TouchableOpacity style={styles.settingDropdown}>
                        <Text style={styles.dropdownText}>Administrateurs</Text>
                        <Ionicons name="chevron-down" size={16} color="#8e9297" />
                      </TouchableOpacity>
                    </View>
                    ...
                    <View style={styles.settingItem}>
                      <View>
                        <Text style={styles.settingLabel}>Qui peut inviter des membres</Text>
                        <Text style={styles.settingSubLabel}>Contrôler qui peut ajouter de nouveaux membres</Text>
                      </View>
                      <TouchableOpacity style={styles.settingDropdown}>
                        <Text style={styles.dropdownText}>Tous les membres</Text>
                        <Ionicons name="chevron-down" size={16} color="#8e9297" />
                      </TouchableOpacity>
                    </View>
                  </View>
                ...
                <View style={styles.settingSection}>
                  <View style={styles.sectionHeader}>
                    <Ionicons name="alert-circle-outline" size={22} color="#ed4245" />
                    <Text style={[styles.settingSectionTitle, { color: '#ed4245' }]}>Actions avancées</Text>
                  </View>
                  
                  <View style={styles.dangerCard}>
                    <View style={styles.dangerItem}>
                      <View>
                        <Text style={styles.dangerItemTitle}>Quitter l'espace de travail</Text>
                        <Text style={styles.dangerItemDescription}>Vous ne pourrez plus accéder aux canaux et messages de cet espace</Text>
                      </View>
                      <TouchableOpacity style={styles.dangerActionButton}>
                        <Ionicons name="exit-outline" size={22} color="#fff" />
                      </TouchableOpacity>
                    </View>
                    
                    {workspace.owner === 'Sophie Martin' && (
                      <>
                        <View style={styles.settingDivider} />
                        <View style={styles.dangerItem}>
                          <View>
                            <Text style={styles.dangerItemTitle}>Supprimer l'espace de travail</Text>
                            <Text style={styles.dangerItemDescription}>Cette action est irréversible. Tous les canaux et messages seront supprimés définitivement.</Text>
                          </View>
                          <TouchableOpacity style={[styles.dangerActionButton, { backgroundColor: '#ed4245' }]}>
          ...
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <View style={styles.modalHeaderContent}>
                ...
                <Text style={styles.modalTitle}>Nouveau canal</Text>
              </View>
              <TouchableOpacity 
            ...
            <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
              <View style={styles.formGroup}>
                <View style={styles.formLabelContainer}>
                  <Ionicons name="text-outline" size={18} color="#8e9297" />
                ...
                <TextInput
                  ...
                  onChangeText={(text) => updateNewChannelForm('name', text)}
                />
              </View>
              
              <View style={styles.formGroup}>
                <View style={styles.formLabelContainer}>
                  <Ionicons name="document-text-outline" size={18} color="#8e9297" />
                ...
                <TextInput
                  ...
                  onChangeText={(text) => updateNewChannelForm('description', text)}
                />
              </View>
              
              <View style={styles.formGroup}>
                <View style={styles.formLabelContainer}>
                  <Ionicons name="eye-outline" size={18} color="#8e9297" />
                ...
                <View style={styles.visibilityOptions}>
                  <TouchableOpacity 
                    style={[
                      styles.visibilityOption,
                      newChannelForm.is_public && styles.selectedVisibility
                    ]}
                    onPress={() => updateNewChannelForm('is_public', true)}
                  >
                    <Ionicons name="globe-outline" size={20} color={newChannelForm.is_public ? '#fff' : '#8e9297'} />
                    <Text style={[
                      styles.visibilityText,
                      newChannelForm.is_public && styles.selectedVisibilityText
                    ]}>Public</Text>
                    {newChannelForm.is_public && <View style={styles.visibilityCheckmark}>
                  ...
                  <TouchableOpacity 
                    ...
                    onPress={() => updateNewChannelForm('is_public', false)}
                  >
                    <Ionicons name="lock-closed-outline" size={20} color={!newChannelForm.is_public ? '#fff' : '#8e9297'} />
                    <Text style={[
                      styles.visibilityText,
                      !newChannelForm.is_public && styles.selectedVisibilityText
                    ]}>Privé</Text>
            ...
            <View style={styles.modalFooter}>
              ...
              <TouchableOpacity 
                style={[styles.createButton, !newChannelForm.name.trim() && styles.disabledButton]}
                onPress={handleCreateChannel}
                disabled={!newChannelForm.name.trim()}
              >
                <Ionicons name="add-circle-outline" size={18} color="#fff" />
                <Text style={styles.createButtonText}>Créer le canal</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
              <View style={styles.formGroup}>
                <View style={styles.formLabelContainer}>
                  <Ionicons name="link-outline" size={18} color="#8e9297" />
                  <Text style={styles.formLabel}>Lien d'invitation</Text>
                </View>
                <View style={styles.inviteLinkContainer}>
                  <Text style={styles.inviteLink}>https://supchat.com/invite/abc123</Text>
                  <TouchableOpacity style={styles.copyButton}>
                    <Ionicons name="copy-outline" size={18} color="#fff" />
                  </TouchableOpacity>
                </View>
                <Text style={styles.inviteLinkHint}>Partagez ce lien pour inviter des personnes à rejoindre cet espace de travail</Text>
              </View>
              ...
              <View style={styles.formGroup}>
                <View style={styles.formLabelContainer}>
                  <Ionicons name="mail-outline" size={18} color="#8e9297" />
                  <Text style={styles.formLabel}>Inviter par email</Text>
                </View>
                <TextInput
                  style={styles.formInput}
              ...
              <View style={styles.formGroup}>
                <View style={styles.formLabelContainer}>
                  <Ionicons name="people-outline" size={18} color="#8e9297" />
                  <Text style={styles.formLabel}>Invitations récentes</Text>
                </View>
                <View style={styles.recentInvitesList}>
                  <View style={styles.recentInviteItem}>
                    <View style={styles.recentInviteInfo}>
                      <Text style={styles.recentInviteEmail}>jean.dupont@exemple.com</Text>
                      <Text style={styles.recentInviteStatus}>En attente • Envoyé il y a 2h</Text>
                    </View>
                    <TouchableOpacity style={styles.recentInviteAction}>
                      <Ionicons name="refresh-outline" size={20} color="#8e9297" />
                    </TouchableOpacity>
                  </View>
                </View>
              </View>
        ...
        <View style={styles.confirmModalOverlay}>
          <View style={styles.confirmModalContent}>
            <View style={styles.confirmModalHeader}>
              <Ionicons name="warning" size={28} color="#ED4245" />
              <Text style={styles.confirmModalTitle}>Supprimer un canal</Text>
            </View>
            
            <View style={styles.confirmModalBody}>
              <Text style={styles.confirmModalText}>
                Êtes-vous sûr de vouloir supprimer <Text style={styles.confirmModalHighlight}>{modalState.channelToRemove?.name || 'ce canal'}</Text> de cet espace de travail ?
              </Text>
              <Text style={styles.confirmModalSubtext}>
                Cette action est irréversible et le canal sera supprimé immédiatement.
              </Text>
            </View>
            ...
            <View style={styles.confirmModalFooter}>
              ...
              <TouchableOpacity 
                ...
                onPress={() => modalState.channelToRemove && handleRemoveChannel(modalState.channelToRemove.uuid)}
              >
      ...
      >
        <View style={styles.confirmModalOverlay}>
          <View style={styles.confirmModalContent}>
            <View style={styles.confirmModalHeader}>
              <Ionicons name="warning" size={28} color="#ED4245" />
              <Text style={styles.confirmModalTitle}>Supprimer un membre</Text>
            </View>
            
            <View style={styles.confirmModalBody}>
              <Text style={styles.confirmModalText}>
                Êtes-vous sûr de vouloir supprimer <Text style={styles.confirmModalHighlight}>{modalState.memberToRemove?.name || modalState.memberToRemove?.username || 'ce membre'}</Text> de cet espace de travail ?
              </Text>
              <Text style={styles.confirmModalSubtext}>
                Cette action est irréversible et le membre perdra immédiatement l'accès à tous les canaux de cet espace de travail.
              </Text>
            </View>
            ...
            <View style={styles.confirmModalFooter}>
              ...
              <TouchableOpacity 
                style={styles.confirmDeleteButton}
                onPress={() => modalState.memberToRemove && handleRemoveMember(modalState.memberToRemove.uuid)}
              >
                <Ionicons name="trash-outline" size={18} color="#fff" />
                <Text style={styles.confirmDeleteButtonText}>Supprimer</Text>
              </TouchableOpacity>
