...
apiClient.interceptors.request.use(
    async (config) => {
        try {
            console.log(
                `Requête API: ${config.method?.toUpperCase()} ${config.url}`,
            );
...
apiClient.interceptors.response.use(
    (response) => {
        console.log(
            `Réponse API (${response.status}): ${response.config.method?.toUpperCase()} ${response.config.url}`,
        );
        return response;
    },
    async (error) => {
        if (error.response) {
            ...
            if (error.response.status === 401) {
                console.log(
                    "Erreur d'authentification, suppression des données de session",
                );
                try {
